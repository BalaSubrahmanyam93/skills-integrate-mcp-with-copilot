...
def require_teacher(teacher_session: Optional[str] = Cookie(default=None)):
    username = active_sessions.get(teacher_session)
    if username is None:
...
def login(credentials: LoginRequest, response: Response):
    ...
    active_sessions[session_id] = credentials.username
    response.set_cookie(
        key=SESSION_COOKIE,
        value=session_id,
        httponly=True,
